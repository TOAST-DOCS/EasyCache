...
#### 強制複製接続解除後の元リージョンからの再接続ブロック

強制複製接続解除を実行すると、元リージョンのキャッシュに昇格が反映されないまま完了する場合があります。このとき、新しいキャッシュに適用された DBセキュリティグループに、元リージョンのノードのIPとサービスポートを許可するルール（0.0.0.0/0 を含む）がある場合にのみ、以下の現象が発生する場合があります。

* 元リージョンが復旧すると、元のキャッシュが昇格したノードを再びリードレプリカに切り替えます。このプロセスで、新しいキャッシュに保存されたデータが失われる場合があります。

これを防ぐには、強制複製接続解除後に以下のいずれかの対処を行います。

* 方法 1. DBセキュリティグループルールの削除
    * 新しいキャッシュの DBセキュリティグループから、元リージョンのノードのIPまたは 0.0.0.0/0 のように、元リージョンのアクセスを許可するルールを削除します。元のキャッシュを整理するまで、当該ルールを再度追加しないでください。
* 方法 2. 元リージョン方向のネットワークパスのブロック
    1. **[Network > Network Interface > ネットワークインターフェイス作成]** で **[仮想IP]** を選択し、ネットワークインターフェイスを作成します。
    2. **[Network > Routing]** でルートテーブルを選択し、下部の **[ルート]** タブでリージョンピアリングをゲートウェイとして使用するルートルールを選択して、**[変更]** をクリックします。
    3. 手順 2 で選択したルートルールのゲートウェイを、手順 1 で作成した仮想IPに変更します。
    * 元のキャッシュを整理した後、ゲートウェイを再びリージョンピアリングに変更すると、通信が復旧します。

!!! danger "注意"
    * 方法 2 で作成した仮想IPポートを他のインスタンスにバインドして使用しないでください。
