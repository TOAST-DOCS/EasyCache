...
### キャッシュ及びノードリスト { #cache-and-node-list }
...
### キャッシュ及びノード詳細 { #cache-and-node-details }
...
### 複製接続解除及び強制複製接続解除 { #remove-replication-connection-and-forced-replication-connection }
...
#### 強制レプリケーション接続除去後の元リージョンからの再接続ブロック

強制レプリケーション接続除去を実行すると、元リージョンのキャッシュに昇格が反映されないまま完了する場合があります。このとき、新しいキャッシュに適用された DB セキュリティグループに、元リージョンのノードの IP とサービスポートを許可するルール（0.0.0.0/0 を含む）がある場合にのみ、以下の現象が発生する場合があります。

* 元リージョンが復旧すると、元のキャッシュが昇格したノードを再びリードレプリカに切り替えます。このプロセスで、新しいキャッシュに保存されたデータが失われる場合があります。

これを防ぐには、強制レプリケーション接続除去後に以下のいずれかの対処を行います。

* 方法 1. DB セキュリティグループルールの削除
    * 新しいキャッシュの DB セキュリティグループから、元リージョンのノードの IP または 0.0.0.0/0 のように、元リージョンのアクセスを許可するルールを削除します。元のキャッシュを整理するまで、当該ルールを再度追加しないでください。
* 方法 2. 元リージョン方向のネットワークパスのブロック
    1. **[Network > Network Interface > ネットワークインターフェイス作成]** で **[仮想 IP]** を選択し、ネットワークインターフェイスを作成します。
    2. **[Network > Routing]** でルートテーブルを選択し、下部の **[ルート]** タブでリージョンピアリングをゲートウェイとして使用するルートルールを選択して、**[変更]** をクリックします。
    3. 手順 2 で選択したルートルールのゲートウェイを、手順 1 で作成した仮想 IP に変更します。
    * 元のキャッシュを整理した後、ゲートウェイを再びリージョンピアリングに変更すると、通信が復旧します。

!!! danger "注意"
    * 方法 2 で作成した仮想 IP ポートを他のインスタンスにバインドして使用しないでください。
    * 方法 2 を適用すると、当該ルートを使用する他のリージョン間通信も同時に遮断されます。

上記の制限は、元リージョンの復旧後に元のキャッシュを削除すると解除できます。元のキャッシュを維持する必要がある場合は、カスタマーセンターにお問い合わせください。
...
### パラメータグループ変更事項適用 { #apply-parameter-group-changes }
...
### 高可用性再設定 { #reset-high-availability }
...
### 高可用性キャッシュ { #high-availability-cache }
...
接続ドメインのIP情報は、新たにフェイルオーバーが行われたノードを基準に更新されます。フローティングIPドメインは変更されませんが、フローティングIPドメインが指すフローティングIPは、新しいマスターを指すように自動的に変更されます。
読み取り専用ドメインの場合、自動フェイルオーバーの実行過程で、読み取りアクセスのためのクライアント通信を妨げないようドメイン情報は直ちには更新されませんが、コンソール画面に**読み取り専用ドメイン更新**というボタンが表示されます。このボタンをクリックすると、クリック時点での読み取り専用ノードのIP情報を基準に更新されます。

!!! tip 「ポイント」
    * フェイルオーバーはマスターノードと同じリージョンに属するノードを対象に実行される機能であるため、異なるリージョンにのみリードレプリカノードを追加している場合は、自動フェイルオーバーに対応していません。
    * フェイルオーバー後に表示される読み取り専用ドメイン更新ボタンは、障害が完全に解消された後にクリックし、最終的な読み取り専用ノードのIPに更新することを推奨します。
    * 実際のフェイルオーバーではなく、マスター変更機能などを通じてIP情報の変更が発生する場合は、自動的に読み取り専用ドメインのIP情報が更新されます。
