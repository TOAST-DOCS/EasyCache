...
* 사용할 수 있는 서비스 포트 범위는 10000 ~ 12000 사이입니다.
...
### 캐시 및 노드 목록 { #cache-and-node-list }
...
### 캐시 및 노드 상세 { #cache-and-node-details }
...
!!! tip "알아두기"
    ...
    * Redis 7.0.7 이상 버전들은 가장 최신 버전으로 업그레이드를 제공합니다.
...
사용자 테넌트 ID, API 비밀번호를 입력한 뒤, {containerName}/{path or fileName} 형식의 데이터 파일 경로를 입력하고 **데이터 가져오기**를 누르면 작업이 시작됩니다.
...
### 복제 연결 제거 및 강제 복제 연결 제거 { #remove-replication-connection-and-forced-replication-connection }
...
#### 강제 복제 연결 제거 후 원본 리전의 재연결 차단

강제 복제 연결 제거를 수행하면 원본 리전의 캐시에 승격 사실이 반영되지 않은 채 완료될 수 있습니다. 이때 새 캐시에 적용된 DB 보안 그룹에 원본 리전 노드의 IP와 서비스 포트를 허용하는 규칙(0.0.0.0/0 포함)이 있는 경우에만 아래 현상이 발생할 수 있습니다.

* 원본 리전이 복구되면 원본 캐시가 승격된 노드를 다시 읽기 복제본으로 전환하며, 이 과정에서 새 캐시에 저장된 데이터가 유실될 수 있습니다.

이를 방지하려면 강제 복제 연결 제거 후 아래 조치 중 하나를 수행하세요.

* 방법 1. DB 보안 그룹 규칙 삭제
    * 새 캐시의 DB 보안 그룹에서 원본 리전 노드의 IP 또는 0.0.0.0/0과 같이 원본 리전의 접근을 허용하는 규칙을 삭제하세요. 원본 캐시를 정리하기 전까지 해당 규칙을 다시 추가하지 마세요.
* 방법 2. 원본 리전 방향 네트워크 경로 차단
    1. **Network > Network Interface > 네트워크 인터페이스 생성**에서 **가상 IP**를 선택해 네트워크 인터페이스를 생성하세요.
    2. **Network > Routing**에서 라우트 테이블을 선택하고, 하단 **라우트** 탭에서 리전 피어링을 게이트웨이로 사용하는 라우트 규칙을 선택해 **변경**을 클릭하세요.
    3. 2에서 선택한 라우트 규칙의 게이트웨이를 1에서 생성한 가상 IP로 변경하세요.
    * 원본 캐시를 정리한 뒤 게이트웨이를 다시 리전 피어링으로 변경하면 통신이 복구됩니다.

!!! danger "주의"
    * 방법 2에서 생성한 가상 IP 포트를 다른 인스턴스에 바인딩해서 사용하면 안 됩니다.
    * 방법 2를 적용하면 해당 라우트를 사용하는 다른 리전 간 통신도 함께 차단됩니다.

위 제한은 원본 리전 복구 후 원본 캐시를 삭제하면 해제할 수 있습니다. 원본 캐시를 유지해야 하는 경우 고객 센터로 문의하세요.
...
### 고가용 재설정 { #reset-high-availability }
...
### 고가용성 캐시 { #high-availability-cache }
...
접속 도메인의 IP 정보가 새로운 장애 조치가 이루어진 노드를 기준으로 갱신되며, 플로팅 IP 도메인은 변경되지 않으나 플로팅 IP 도메인이 가리키는 플로팅 IP는 새로운 마스터를 가리키도록 자동으로 변경됩니다.
읽기 전용 도메인의 경우 자동 장애 조치 수행 과정에서 읽기 접근을 위한 클라이언트들의 통신을 방해하지 않기 위해 도메인 정보가 바로 갱신되지는 않으나 콘솔 화면에 **읽기 전용 도메인 갱신**이라는 버튼이 노출되며, 해당 버튼을 클릭하면 클릭 시점에 읽기 전용 노드들의 IP 정보를 기준으로 갱신됩니다.

!!! tip "알아두기"
    * 장애 조치는 마스터 노드와 같은 리전에 속한 노드들을 대상으로 수행하는 기능이므로, 다른 리전에만 읽기 복제본 노드를 추가할 경우는 자동 장애 조치를 지원하지 않습니다.
    * 장애 조치 후 노출되는 읽기 전용 도메인 갱신 버튼의 경우 장애가 완전히 해소가 된 뒤에 클릭하여 최종적인 읽기 전용 노드들의 IP로 갱신하는 것을 권장합니다.
    * 실제 장애 조치가 아닌 마스터 변경 기능 등을 통해 IP 정보 변경이 발생하는 경우에는 자동으로 읽기 전용 도메인의 IP 정보가 갱신됩니다.
