...
* 방법 2. 원본 리전 방향 네트워크 경로 차단
    ...
    2. **Network > Routing**에서 라우트 테이블을 선택하고, 하단 **라우트** 탭에서 리전 피어링을 게이트웨이로 사용하는 라우트 룰을 선택해 **변경**을 클릭한 뒤, 게이트웨이를 1에서 생성한 가상 IP로 변경하세요.
...
위 제한은 원본 리전 복구 후 원본 캐시를 삭제하면 해제할 수 있습니다. 원본 캐시를 유지해야 하는 경우 고객 센터로 문의해 주세요.
