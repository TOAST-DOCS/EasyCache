...
You can prepare in advance to recover your cache in case of failure situations. You can perform backups in the console whenever needed, or configure backups to be performed periodically. While backups are being performed, performance degradation of the cache's master node may occur. To avoid affecting the service, we recommend that you perform backups during times when the service load is low. If you don't want performance degradation caused by backups, you can also use a high-availability configuration.

!!! tip "Note"
    Valkey creates .rdb files when performing backups. To restore using the created backup, we recommend that you use a cache with an engine version that is compatible with the backup.
