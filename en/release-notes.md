...
- Fixed an issue where DB security group rule changes were not applied to read replica nodes in other regions
- Fixed an issue where an OS version upgrade failed for a cache with a parameter group sync status of `UNSYNCED`
- Fixed an issue where an OS version upgrade failed for a single node cache
...
- Fixed a bug where the cache status became abnormal when repeatedly adding and deleting read replica nodes in another region
